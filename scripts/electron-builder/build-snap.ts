...
async function postProcessSnapPackage(snapFile: string) {
    ...
    LOG(LOG_LEVELS.title(
        `Copying ${LOG_LEVELS.value(String(dictionaryFiles.length))} dictionaries to ${LOG_LEVELS.value(unSquashedSnapHunspellDir)}`,
    ));
    ...
    await execShell(["mv", ["--force", snapFile, `snap.bak`]]);
