...
export function registerWebFolderFileProtocol(ctx: Context, session: Session): void {
    ...
    const registered = session.protocol.registerFileProtocol(scheme, (request, callback) => {
        const url = new URL(request.url);
        const resource = path.normalize(path.join(webPath, url.host, url.pathname));
...
const readFileAndInjectApiUrl = async (
    fileLocation: string,
    {entryUrl: accountEntryUrl, requestUrl, config$}: Readonly<Pick<AccountConfig, "entryUrl">> & {
        requestUrl: URL;
...
export async function registerAccountSessionProtocols(
    ...
    const registered = session.protocol.registerStreamProtocol(scheme, async (request, callback) => {
        ...
        if (resourceLocation.startsWith(path.join(directory, PROVIDER_REPO_MAP["proton-drive"].basePath, "downloadSW."))) {
            ...
            Object.assign(callbackResponse.headers ??= {}, {"Service-Worker-Allowed": "/", "Service-Worker": "script"});
        }
