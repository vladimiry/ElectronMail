        ...
        from(
            (async () => {
                ...
                const resolveBootstrappedMessageIds = async (): Promise<DeepReadonly<Array<{ ID: string }>>> => {
                    return resolveIpcMainApi({timeoutMs: ONE_SECOND_MS * 30, logger})(
                ...
                if (
                    ...
                    let oldestMailData = await resolveIpcMainApi(
                        {timeoutMs: ONE_SECOND_MS, logger},
                    )("dbGetAccountBootstrapOldestRawMailMetadata")({login});
                    ...
                    while (!fetchResponse || fetchResponse.Messages.length) {
                        const {Messages: messages} = (
                            fetchResponse = await providerApi.message.queryMessageMetadata({
...

                {
                    ...
                    for (const bootstrappedMessageIdsChunk of chunk(bootstrappedMessageIds, PROTON_MAX_CONCURRENT_FETCH)) {
                        await Promise.all(
                            bootstrappedMessageIdsChunk.map(async ({ID: storedMessageId}) => {
