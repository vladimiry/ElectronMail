...
async function resolveCommand(): Promise<{command: string}> {
    const binaryBundle = (() => {
        const fileNames = {
            darwin: {
                postfix: "macos",
                sha256: "61d28a3f24dc3beb74e8b6d1de28bca760685b81bff907142c51e58f6bc8ff87",
            },
            linux: {
                postfix: "linux-x64-static",
                sha256: "ee3292c87a2d77bbe92f101a2d7b2723de811279ceac5d8257f7e95ddc982579",
            },
            win32: {
                postfix: "windows-x64-static.exe",
                sha256: "09d923348d083c130e8e26f990ba4c90022a2b26e6a9f0c55ac78420f84089f8",
            },
        } as const;
...
async function uploadFileArg(): Promise<{downloadUrl: string}> {
    ...
    const linuxNotX64 = os.platform() === "linux" && os.arch() !== "x64";
    const {stdout: downloadUrl} = await execShell([linuxNotX64 ? "/usr/bin/qemu-x86_64-static" : command, [
        ...(linuxNotX64 ? [command] : []),
        "upload",
        "--no-interact",
        "--incognito",
