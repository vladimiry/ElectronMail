...
export const initAppWithTestContext = async (
    options: {readonly initial: boolean; readonly allowDestroyedExecutionContext?: boolean; readonly testContext?: TestContext},
    ...
    await (async (): Promise<void> => {
        const el = await testContext.firstWindowPage.waitForSelector(".e2e-stub-element", {
            timeout: ONE_SECOND_MS * 3,
            state: options.initial ? "visible" : "attached",
        });
        ...
        {
            const bannedUserAgentWords = (["electron", PRODUCT_NAME, PACKAGE_NAME, BINARY_NAME, PACKAGE_VERSION] as readonly string[]).map((
                banned,
            ) => banned.toLowerCase());
        ...
        { // after all
            if (fs.existsSync(testContext.logFilePath)) {
                await new Promise((resolve, reject) => {
                    const stream = byline.createStream(fs.createReadStream(testContext.logFilePath));
