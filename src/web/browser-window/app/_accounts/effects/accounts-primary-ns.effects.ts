...
export class AccountsPrimaryNsEffects {
    readonly common$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType(ACCOUNTS_ACTIONS.SetupCommonNotificationChannel),
                mergeMap(({payload, ...action}) => {
                    const {webView, finishPromise, account: {accountIndex, accountConfig: {login, entryUrl}}} = payload;
                    const logger = curryFunctionMembers(_logger, `[${action.type}][${accountIndex}]`);
                    const dispose$ = from(finishPromise).pipe(tap(() => {
                        logger.info("dispose");
                        this.store.dispatch(
                            this.accountsService.generateCommonNotificationsStateResetAction({login, optionalAccount: true}),
                        );
                    }));
                    ...
                    return merge(
                        from(
                            this.api.primaryCommonWebViewClient({webView}, {finishPromise})("notification")({
                                login,
                                apiEndpointOrigin: this.core.parseSessionStorageOrigin({entryUrl}).apiEndpointOrigin,
                                accountIndex,
                            }),
                        ).pipe(
                            mergeMap((notification) => of(ACCOUNTS_ACTIONS.Patch({login, patch: {notifications: notification}}))),
                        ),
                        // WARN: only needs to be processed for the account that already got a webview created (loaded account page case)
                        // this processing intended to trigger the "webview.partition" update via
                        // the "unload" action (component re-creating)
                        this.store.pipe(
    ...
    readonly mailEffect$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType(ACCOUNTS_ACTIONS.SetupMailNotificationChannel),
                mergeMap(({payload, ...action}) => {
                    ...
                    const dispose$ = from(finishPromise).pipe(tap(() => {
                        logger.info("dispose");
                        this.store.dispatch(
                            this.accountsService.generateMailNotificationsStateResetAction({login, optionalAccount: true}),
                        );
                    }));
                    ...
                    return merge(
                        from(
                            this.api.primaryMailWebViewClient({webView}, {finishPromise})("notification")({
                                entryApiUrl: resolveProtonApiOrigin(
