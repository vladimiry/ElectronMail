...
type ActionsRecord<P extends PropsRecord = PropsRecord> = {
    [T in Extract<keyof P, string>]: P[T] extends ActionCreatorProps<RawObject>
        ? ActionCreator<T, (props: P[T]["_p"] /* & NotAllowedCheck<P[T]["_p"]> */) => {payload: P[T]["_p"]} & TypedAction<T>>
...
type Cases<Record, A> = {
    [T in keyof Record]: (
        value: Record[T] extends {payload: infer P} ? P
            : void,
    ) => A;
};
...
export const propsRecordToActionsRecord = <P extends PropsRecord>(value: P, {prefix}: {prefix: string}):
    & ActionsRecord<P>
    ...
    & {is: (value: UnionOf<ActionsRecord>) => value is UnionOf<ActionsRecord<P>>} =>
{
    ...
    return {
        ...Object.entries(value).reduce((accumulator, [key, actionCreatorProps]) => {
