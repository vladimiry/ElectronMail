...
export const {name: PROJECT_NAME, version: PROJECT_VERSION} // eslint-disable-next-line @typescript-eslint/no-var-requires
    = require("package.json") as { name: string, version: string }; // tslint:disable-line: no-import-zones no-var-requires
...
function resolveEntryUrlIndexByValue(entryUrl: string): number {
    const index = ACCOUNTS_CONFIG.tutanota.entryUrl.findIndex(({value}) => value === entryUrl);
    ...
    const workflow = {
        ...
        async addAccount(
            ...
            await t.context.app.client.execute(
                (_selector) => {
                    const el = document.querySelector<HTMLButtonElement>(_selector);
                    if (!el) {
                        throw new Error(`Failed to resolve element using "${String(_selector)}" selector`);
                    }
...
export async function initApp(t: ExecutionContext<TestContext>, options: { initial: boolean }): Promise<TestContext["workflow"]> {
    ...
    t.false(
        // tslint:disable-next-line:await-promise eslint-disable-next-line @typescript-eslint/await-thenable
