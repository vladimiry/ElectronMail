...
async function resolveBoundsToRestore(ctx: Context, currentBounds: Readonly<Rectangle>): Promise<Rectangle> {
    const config = await lastValueFrom(ctx.config$.pipe(first()));
    const {window: {bounds: savedBounds}} = config;
    const x = Math.max(savedBounds.x ?? currentBounds.x, 0);
    const y = Math.max(savedBounds.y ?? currentBounds.y, 0);
    ...
    const width = Math.max(Math.min(savedBounds.width, allDisplaysSummarySize.width), 100);
    const height = Math.max(Math.min(savedBounds.height, allDisplaysSummarySize.height), 100);
    const result = {
        width,
        height,
        x: Math.min(x, Math.max(allDisplaysSummarySize.width - width, 0)),
        y: Math.min(y, Math.max(allDisplaysSummarySize.height - height, 0)),
    } as const;
...
async function keepBrowserWindowState(ctx: Context, browserWindow: Electron.BrowserWindow): Promise<void> {
    ...
    const saveWindowStateHandler = async (): Promise<void> => {
        await ctx.configStoreQueue.q(async () => {
            ...
            if (isDeepEqual(storedWindowConfig, newWindowConfig)) {
                return;
            }
    ...
    const saveWindowStateHandlerDebounced = ((): () => void => {
