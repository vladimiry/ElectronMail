...
is an [Electron](https://electronjs.org)-based unofficial desktop client for [Tutanota](https://tutanota.com/) end-to-end encrypted email provider. The app aims to provide enhanced desktop user experience enabling features that are not supported by the official in-browser web clients. 
...
- :closed_lock_with_key: **Built-in/prepackaged web clients**. The built-in web clients are built from source code, see respective official [Tutanota](https://github.com/tutao/tutanota) repository. See [79](https://github.com/vladimiry/ElectronMail/issues/79) and [80](https://github.com/vladimiry/ElectronMail/issues/80) issues for details.
...
- Some native modules require node prebuilds files compiling and for that Python and C++ compiler need to be installed on your system:
  - On `Windows`: the simplest way to install all the needed stuff on Windows is to run `npm install --global --production windows-build-tools` CLI command. [Tutanota](https://github.com/tutao/tutanota) web client project requires bash for building, so a few more steps need to be fulfilled:
