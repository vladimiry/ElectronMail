...
import {HttpApi, resolveStandardSetupPublicApi} from "src/electron-preload/webview/primary/lib/provider-api/standart-setup-internals";
    ...
    return (async (): Promise<ProviderApi> => {
        ...
        const providerApi: ProviderApi = {
            _custom_: {
                ...
                buildConversationsCountApiUrlTester(/*{entryApiUrl}*/) {
                    const substr = "/mail/v4/conversations/count";
                    return (url) => url.endsWith(substr) || url.includes(`${substr}?`);
                },
